...
multiple roles, fresh values, encrypted components, an active intruder, secrecy,
agreement, and messages whose legality depends on the preceding trace.
...
\section{Secrecy and agreement become theorems}

\index{secrecy}
\index{agreement}
A secrecy theorem quantifies over every accepted trace, not merely the happy-path
scenario. For Yahalom:

\begin{lstlisting}
theorem yahalom_session_key_secret
    (trace : Trace WireEvent)
    (accepted : yahalom.Accepts trace)
    (honest : Uncompromised [.alice, .bob, .server]) :
    not (Derives (knowledgeOf .ivo trace) (sessionKey accepted.kab)) := by
  -- proof uses the fixed symbolic-crypto library
  exact secrecyFromKeyOwnership accepted honest
...
The example also distinguishes ordinary secrecy from \emph{strong secrecy}: an
attacker should not be able to distinguish executions merely by substituting one
candidate session key for another. That is a relational property over pairs of
trace distributions, beyond a single CTL state formula, and should be represented
as a separate theorem family.
...
\begin{enumerate}
  ...
  \item trace secrecy, agreement, and relational strong-secrecy theorem schemas;
