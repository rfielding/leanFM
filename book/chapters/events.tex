...
\texttt{prevId} records an immediate causal claim; \texttt{from} and \texttt{to}
name the parties; and \texttt{kind} supplies a stable discriminator. The payload is
protocol-specific and carries the parameters that flow from one production to the
next. A timestamp is useful, but order and causality must not be silently reduced
to wall-clock order.
