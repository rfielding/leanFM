...
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[->,>=Latex,node distance=2.7cm,on grid,auto,
  every state/.style={draw,minimum size=1.05cm}]
  \node[state,initial] (start) {start};
  \node[state] (asked) [right=of start] {asked};
  \node[state] (offered) [right=of asked] {offered};
  \node[state,accepting] (accepted) [above right=1.4cm and 2.5cm of offered] {accepted};
  \node[state,accepting] (declined) [below right=1.4cm and 2.5cm of offered] {declined};
  \path
    (start) edge node {quote requested} (asked)
    (asked) edge node {quote offered} (offered)
    (offered) edge node {quote accepted} (accepted)
    (offered) edge node[below] {quote declined} (declined);
\end{tikzpicture}
