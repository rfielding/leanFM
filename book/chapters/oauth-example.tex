...
def userDecision : Choice UserDecision :=
  choose .user "consent decision"
    [ weighted (87 / 100) .grant
    , weighted (13 / 100) .deny ]

def authorizationServer
    (input : UserDecision) : Scenario :=
  match input with
...
After the user's choice, every transition may have probability one and the global
model is still probabilistic. The grant branch carries probability mass $0.87$
through the user's outbound queue, transport, the server's inbound queue, and the
deterministic grant response. The denial branch similarly carries mass $0.13$.
Deterministic processing transports probability mass; it does not remove it.
