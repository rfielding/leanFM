...
def liveSubjectGraph : String :=
  ...
  "function groupChildren(n){const msgs=(model.messages||[]).map(msgParts),props=model.properties||[],reducers=model.reducers||[];if(n.id==='pre')return['entry request','visible preconditions','queue/readiness'];if(n.id==='exchange')return['start'].concat(msgs.map(m=>'after '+m.proto)).concat(['waiting for reply','terminal']);if(n.id==='terminal')return['postcondition','cleanup task','terminal observation'];if(n.id==='annotations')return['actors: '+(model.actors||[]).join(', ')].concat(props.slice(0,3)).concat(reducers.slice(0,2));return[];}" ++
  ...
  "function childState(parent,id,i,total){const key=parent.id+'|'+id;let c=childById.get(key);const s=groupSize(parent),angle=-Math.PI/2+i*Math.PI*2/Math.max(1,total),r=Math.max(70,Math.min(s.w,s.h)/3),tx=parent.x+Math.cos(angle)*r,ty=parent.y+Math.sin(angle)*r;if(!c){c={id,key,x:tx,y:ty,vx:0,vy:0,pinned:false,parent:parent.id};childById.set(key,c);}if(!c.pinned){c.vx+=(tx-c.x)*0.025;c.vy+=(ty-c.y)*0.025;}for(const other of childById.values()){if(other===c||!other.key.startsWith(parent.id+'|'))continue;let dx=c.x-other.x,dy=c.y-other.y;if(Math.abs(dx)<1&&Math.abs(dy)<1){dx=4;dy=3;}const ox=300-Math.abs(dx),oy=70-Math.abs(dy);if(ox>0&&oy>0&&!c.pinned){if(ox<oy)c.vx-=Math.sign(dx||1)*ox*0.06;else c.vy-=Math.sign(dy||1)*oy*0.06;}}if(!c.pinned){c.vx*=0.62;c.vy*=0.62;c.x+=c.vx;c.y+=c.vy;}c.x=Math.max(parent.x-s.w/2+145,Math.min(parent.x+s.w/2-145,c.x));c.y=Math.max(parent.y-s.h/2+72,Math.min(parent.y+s.h/2-34,c.y));return c;}" ++
  "function drawNode(n){const s=groupSize(n),x=n.x-s.w/2,y=n.y-s.h/2;ctx.fillStyle=openGroups.has(n.id)?'#111827':'#000';ctx.strokeStyle=n.color;ctx.lineWidth=2;ctx.fillRect(x,y,s.w,s.h);ctx.strokeRect(x,y,s.w,s.h);ctx.fillStyle='#fff';ctx.font='15px sans-serif';ctx.textAlign='center';ctx.fillText(n.label,n.x,y+26);ctx.font='12px sans-serif';ctx.fillText(openGroups.has(n.id)?'open: click to collapse':'closed: click to expand',n.x,y+47);hit.push({kind:'group',id:n.id,x,y,w:s.w,h:s.h});if(openGroups.has(n.id)){const labels=groupChildren(n),kids=labels.map((st,i)=>childState(n,st,i,labels.length));kids.forEach((c,i)=>{const color=i===0?'#93c5fd':i===labels.length-1?'#22c55e':n.color;rect(c.x-130,c.y-20,260,40,labels[i],color);hit.push({kind:'child',id:c.key,x:c.x-130,y:c.y-20,w:260,h:40});});if(n.id==='exchange'){const msgs=(model.messages||[]).map(msgParts);for(let i=0;i<msgs.length&&i+1<kids.length;i++){const a=kids[i],b=kids[i+1],m=msgs[i];arrow(a.x,a.y+22,b.x,b.y-22,'#fbbf24');labelBox((a.x+b.x)/2+135,(a.y+b.y)/2,m.src+' -> '+m.dst+' '+m.proto,'#fbbf24',270);}}}}" ++
  ...
  "function point(ev){const r=cv.getBoundingClientRect();return{x:(ev.clientX-r.left)*cv.width/r.width,y:(ev.clientY-r.top)*cv.height/r.height};}function hitAt(p){return hit.filter(h=>p.x>=h.x&&p.x<=h.x+h.w&&p.y>=h.y&&p.y<=h.y+h.h).sort((a,b)=>(a.w*a.h)-(b.w*b.h))[0];}cv.addEventListener('pointerdown',ev=>{const p=point(ev),h=hitAt(p);if(!h)return;if(h.kind==='child'){const c=childById.get(h.id);if(!c)return;c.pinned=true;drag={kind:'child',node:c,offX:p.x-c.x,offY:p.y-c.y,moved:false};}else{const n=nodes.find(x=>x.id===h.id);if(!n)return;n.pinned=true;drag={kind:'group',node:n,offX:p.x-n.x,offY:p.y-n.y,moved:false};}cv.setPointerCapture(ev.pointerId);});cv.addEventListener('pointermove',ev=>{if(!drag)return;const p=point(ev);drag.moved=true;drag.node.x=p.x-drag.offX;drag.node.y=p.y-drag.offY;if(drag.kind==='group')clamp(drag.node);});cv.addEventListener('pointerup',ev=>{if(drag){try{cv.releasePointerCapture(ev.pointerId);}catch(e){}if(!drag.moved&&drag.kind==='group'){openGroups.has(drag.node.id)?openGroups.delete(drag.node.id):openGroups.add(drag.node.id);localStorage.setItem('leanfm.subject.open',JSON.stringify([...openGroups]));}drag=null;}});cv.addEventListener('dblclick',ev=>{const p=point(ev),h=hitAt(p);if(!h)return;if(h.kind==='child'){const c=childById.get(h.id);if(c)c.pinned=false;}else{const n=nodes.find(x=>x.id===h.id);if(n)n.pinned=false;}});" ++
