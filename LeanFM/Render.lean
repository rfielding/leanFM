...
def liveSubjectGraph : String :=
  ...
  "const actors=model.actors||[],msgs=(model.messages||[]).map(msgParts),props=model.properties||[],reducers=model.reducers||[];group(24,24,1132,106,'requirement super-state');rect(48,58,310,42,(model.label||model.task||model.id||'draft'),'#22c55e');rect(420,58,188,42,'observable state only','#93c5fd');rect(660,58,188,42,'message transitions','#fbbf24');rect(900,58,188,42,'CTL over coarse SM','#22c55e');arrow(608,79,660,79,'#777');arrow(848,79,900,79,'#777');" ++
  "group(24,154,792,500,'nested state machine');if(!msgs.length){rect(258,360,330,56,'Ask the assistant to sketch Kerberos or a task.','#93c5fd');return;}const states=[];states.push({label:'start / preconditions',color:'#93c5fd'});msgs.forEach((m,i)=>states.push({label:'after '+m.proto,color:i===msgs.length-1?'#22c55e':'#fbbf24'}));states.push({label:'terminal / cleanup',color:'#22c55e'});const x=276,w=260,h=46,y0=208,gap=54;states.forEach((s,i)=>rect(x,y0+i*gap,w,h,s.label,s.color));msgs.forEach((m,i)=>{const y1=y0+i*gap+h,y2=y0+(i+1)*gap;arrow(x+w/2,y1,x+w/2,y2,'#fbbf24');labelBox(x+w+170,(y1+y2)/2,m.src+' -> '+m.dst+' '+m.proto,'#fbbf24',300);});const ly=y0+msgs.length*gap+h,ty=y0+(msgs.length+1)*gap;arrow(x+w/2,ly,x+w/2,ty,'#777');labelBox(x+w+170,(ly+ty)/2,'terminal cleanup / task completes','#777',300);" ++
  "group(844,154,312,500,'annotations');rect(872,198,256,44,'actors: '+actors.join(', '),'#93c5fd');rect(872,262,256,44,'every message carries src,dst,payload','#fbbf24');rect(872,326,256,44,'reply uses visible return address','#fbbf24');props.slice(0,4).forEach((p,i)=>rect(872,398+i*58,256,42,p,'#22c55e'));reducers.slice(0,2).forEach((r,i)=>rect(872,630+i*38,256,30,r,'#a78bfa'));" ++
  "}" ++
