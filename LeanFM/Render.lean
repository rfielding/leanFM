...
def liveSubjectGraph : String :=
  ...
  "function arrow(x1,y1,x2,y2,color,dashed){ctx.strokeStyle=color||'#93c5fd';ctx.fillStyle=color||'#93c5fd';ctx.lineWidth=2;ctx.setLineDash(dashed?[8,6]:[]);ctx.beginPath();ctx.moveTo(x1,y1);ctx.lineTo(x2,y2);ctx.stroke();ctx.setLineDash([]);const a=Math.atan2(y2-y1,x2-x1);ctx.beginPath();ctx.moveTo(x2,y2);ctx.lineTo(x2-12*Math.cos(a-.45),y2-12*Math.sin(a-.45));ctx.lineTo(x2-12*Math.cos(a+.45),y2-12*Math.sin(a+.45));ctx.closePath();ctx.fill();}" ++
  ...
  "function concreteEdges(){const msgs=(model.messages||[]).map(msgParts),out=[['pre|entry request','exchange|state 0: ready','instantiate task FSM from queued request','struct']];for(let i=0;i<msgs.length;i++){const m=msgs[i];out.push(['exchange|state '+i+': '+(i===0?'ready':'after '+msgs[i-1].proto),'exchange|state '+(i+1)+': after '+m.proto,m.src+' -> '+m.dst+' '+m.proto,'msg']);}if(msgs.length)out.push(['exchange|state '+msgs.length+': after '+msgs[msgs.length-1].proto,'terminal|postcondition','terminal cleanup / task completes','struct']);out.push(['terminal|postcondition','annotations|CTL over visible states','optional CTL / reducers over visible fields','optional']);return out;}" ++
  ...
  "function separateRects(){for(let pass=0;pass<36;pass++){let moved=false;for(let i=0;i<nodes.length;i++)for(let j=i+1;j<nodes.length;j++){const a=nodes[i],b=nodes[j],as=groupSize(a),bs=groupSize(b);let dx=b.x-a.x,dy=b.y-a.y;if(Math.abs(dx)<1&&Math.abs(dy)<1){dx=11;dy=7;}const ox=(as.w+bs.w)/2+150-Math.abs(dx),oy=(as.h+bs.h)/2+150-Math.abs(dy);if(ox>0&&oy>0){moved=true;if(ox<oy){const sgn=Math.sign(dx||1),am=a.pinned?0:(b.pinned?1:.5),bm=b.pinned?0:(a.pinned?1:.5);if(!a.pinned)a.x-=ox*am*sgn;if(!b.pinned)b.x+=ox*bm*sgn;a.vx=0;b.vx=0;}else{const sgn=Math.sign(dy||1),am=a.pinned?0:(b.pinned?1:.5),bm=b.pinned?0:(a.pinned?1:.5);if(!a.pinned)a.y-=oy*am*sgn;if(!b.pinned)b.y+=oy*bm*sgn;a.vy=0;b.vy=0;}clamp(a);clamp(b);}}if(!moved)break;}}" ++
  ...
  "function clampSubjectChild(c,parent){const s=groupSize(parent);c.x=Math.max(parent.x-s.w/2+145,Math.min(parent.x+s.w/2-145,c.x));c.y=Math.max(parent.y-s.h/2+76,Math.min(parent.y+s.h/2-46,c.y));}" ++
  "function separateSubjectChildren(parent,kids){for(let pass=0;pass<36;pass++){let moved=false;for(let i=0;i<kids.length;i++)for(let j=i+1;j<kids.length;j++){const a=kids[i],b=kids[j];let dx=b.x-a.x,dy=b.y-a.y;if(Math.abs(dx)<1&&Math.abs(dy)<1){dx=5;dy=3;}const ox=292-Math.abs(dx),oy=58-Math.abs(dy);if(ox>0&&oy>0){moved=true;if(ox<oy){const sgn=Math.sign(dx||1),am=a.pinned?0:(b.pinned?1:.5),bm=b.pinned?0:(a.pinned?1:.5);if(!a.pinned)a.x-=ox*am*sgn;if(!b.pinned)b.x+=ox*bm*sgn;a.vx=0;b.vx=0;}else{const sgn=Math.sign(dy||1),am=a.pinned?0:(b.pinned?1:.5),bm=b.pinned?0:(a.pinned?1:.5);if(!a.pinned)a.y-=oy*am*sgn;if(!b.pinned)b.y+=oy*bm*sgn;a.vy=0;b.vy=0;}clampSubjectChild(a,parent);clampSubjectChild(b,parent);}}if(!moved)break;}}" ++
  "function childState(parent,id,i,total){const key=parent.id+'|'+id;let c=childById.get(key);const s=groupSize(parent),cols=Math.max(1,Math.ceil(Math.sqrt(total))),col=i%cols,row=Math.floor(i/cols),tx=parent.x+(col-(cols-1)/2)*330,ty=parent.y-s.h/2+96+row*96;if(!c){c={id,key,x:tx,y:ty,vx:0,vy:0,pinned:false,parent:parent.id};childById.set(key,c);}if(!c.pinned){c.vx+=(tx-c.x)*0.014;c.vy+=(ty-c.y)*0.014;c.vx=Math.max(-8,Math.min(8,c.vx*0.38));c.vy=Math.max(-8,Math.min(8,c.vy*0.38));c.x+=c.vx;c.y+=c.vy;}clampSubjectChild(c,parent);return c;}" ++
  "function hasMsgIncident(key){return concreteEdges().some(e=>e[3]==='msg'&&(e[0]===key||e[1]===key));}" ++
  "function drawNode(n){const s=groupSize(n),x=n.x-s.w/2,y=n.y-s.h/2;ctx.fillStyle=openGroups.has(n.id)?'#111827':'#000';ctx.strokeStyle=n.color;ctx.lineWidth=2;ctx.fillRect(x,y,s.w,s.h);ctx.strokeRect(x,y,s.w,s.h);ctx.fillStyle='#fff';ctx.font='15px sans-serif';ctx.textAlign='center';ctx.fillText(n.label,n.x,y+26);ctx.font='12px sans-serif';ctx.fillText(openGroups.has(n.id)?'open: click to collapse':'closed: click to expand',n.x,y+47);hit.push({kind:'group',id:n.id,x,y,w:s.w,h:s.h});if(openGroups.has(n.id)){const labels=groupChildren(n),kids=labels.map((st,i)=>childState(n,st,i,labels.length));separateSubjectChildren(n,kids);kids.forEach((c,i)=>{const color=i===0?'#93c5fd':i===labels.length-1?'#22c55e':n.color;rect(c.x-130,c.y-20,260,40,labels[i],color);if(!hasMsgIncident(c.key)){ctx.fillStyle='#9ca3af';ctx.font='10px sans-serif';ctx.textAlign='right';ctx.fillText('no message edge',c.x+124,c.y+17);}hit.push({kind:'child',id:c.key,x:c.x-130,y:c.y-20,w:260,h:40});});}}" ++
  ...
  "function edgeStyle(kind){return kind==='msg'?{c:'#fbbf24',d:false,name:'message send'}:kind==='optional'?{c:'#9ca3af',d:true,name:'optional analysis'}:{c:'#64748b',d:true,name:'structural'};}" ++
  "function drawConcreteEdges(){const seen=new Map();for(const e of concreteEdges()){const a=endpoint(e[0]),b=endpoint(e[1]);if(!a||!b)continue;const st=edgeStyle(e[3]);const collapsed=a.kind==='group'&&b.kind==='group'&&a.id===b.id;if(collapsed){const key=a.id+'|'+e[3];seen.set(key,{gid:a.id,kind:e[3],labels:(seen.get(key)?.labels||[]).concat(e[2])});continue;}const p1=portFrom(a,b),p2=portFrom(b,a);arrow(p1.x,p1.y,p2.x,p2.y,st.c,st.d);labelBox((p1.x+p2.x)/2,(p1.y+p2.y)/2-16,st.name+': '+e[2],st.c,300);}for(const item of seen.values()){const n=nodes.find(x=>x.id===item.gid);if(!n)continue;const st=edgeStyle(item.kind);labelBox(n.x,n.y+18,item.labels.length+' '+st.name+' edge(s) inside: '+item.labels.slice(0,2).join(' | '),st.c,340);}}" ++
  "function drawLegend(){ctx.textAlign='left';ctx.font='12px sans-serif';ctx.fillStyle='#fbbf24';ctx.fillText('solid amber = concrete src -> dst message send',18,24);ctx.fillStyle='#9ca3af';ctx.fillText('dashed gray = structural or optional non-message edge',18,42);}" ++
  "function render(){if(!nodes.length)build();if(!drag)resizeCanvas();stepForce();ctx.fillStyle='#111';ctx.fillRect(0,0,cv.width,cv.height);title.textContent=model.label||model.task||model.id||'Current draft';summary.textContent='Persistent browser model: '+(model.actors||[]).length+' actors, '+(model.messages||[]).length+' visible messages, '+(model.properties||[]).length+' properties. Click composite states to open or close them.';hit=[];for(const n of nodes)drawNode(n);drawConcreteEdges();drawLegend();requestAnimationFrame(render);}" ++
...
def aggregateGraphAnimation : String :=
  ...
  "function resolveOverlaps(){for(let pass=0;pass<48;pass++){let moved=false;for(let i=0;i<simGroups.length;i++){for(let j=i+1;j<simGroups.length;j++){const a=simGroups[i],b=simGroups[j],as=groupSize(a),bs=groupSize(b);let dx=b.x-a.x,dy=b.y-a.y;if(Math.abs(dx)<0.1&&Math.abs(dy)<0.1){dx=(j-i)*9;dy=7;}const ox=(as.w+bs.w)/2+86-Math.abs(dx),oy=(as.h+bs.h)/2+86-Math.abs(dy);if(ox>0&&oy>0){moved=true;if(ox<oy){const sgn=Math.sign(dx||1),am=a.pinned?0:(b.pinned?1:.5),bm=b.pinned?0:(a.pinned?1:.5);if(!a.pinned)a.x-=ox*am*sgn;if(!b.pinned)b.x+=ox*bm*sgn;a.vx=0;b.vx=0;}else{const sgn=Math.sign(dy||1),am=a.pinned?0:(b.pinned?1:.5),bm=b.pinned?0:(a.pinned?1:.5);if(!a.pinned)a.y-=oy*am*sgn;if(!b.pinned)b.y+=oy*bm*sgn;a.vy=0;b.vy=0;}clampGroup(a);clampGroup(b);}}}if(!moved)break;}}" ++
  ...
  "function drawArrow(ctx,x1,y1,x2,y2,color,width,dashed){ctx.strokeStyle=color;ctx.fillStyle=color;ctx.lineWidth=width;ctx.setLineDash(dashed?[8,6]:[]);ctx.beginPath();ctx.moveTo(x1,y1);ctx.lineTo(x2,y2);ctx.stroke();ctx.setLineDash([]);const a=Math.atan2(y2-y1,x2-x1);ctx.beginPath();ctx.moveTo(x2,y2);ctx.lineTo(x2-12*Math.cos(a-.45),y2-12*Math.sin(a-.45));ctx.lineTo(x2-12*Math.cos(a+.45),y2-12*Math.sin(a+.45));ctx.closePath();ctx.fill();}" ++
