...
def liveSubjectGraph : String :=
  "<section><h2>Current Requirement</h2><div class=\"subjectPanel\"><canvas id=\"subjectGraph\" width=\"1100\" height=\"620\"></canvas><div><h3 id=\"subjectTitle\">No active draft</h3><p id=\"subjectSummary\">Ask the assistant to sketch a protocol or task.</p><div id=\"subjectArtifacts\" class=\"artifactPanel\"></div></div></div></section>" ++
  "<script>" ++
  ...
  "function lines(text,max){const words=String(text).split(/\\s+/),out=[];let line='';for(const w of words){if((line+' '+w).trim().length>max){if(line)out.push(line);line=w;}else line=(line+' '+w).trim();}if(line)out.push(line);return out;}" ++
  ...
  "function rect(x,y,w,h,label,color){ctx.fillStyle='#000';ctx.strokeStyle=color||'#fff';ctx.lineWidth=2;ctx.fillRect(x,y,w,h);ctx.strokeRect(x,y,w,h);ctx.fillStyle='#fff';ctx.font='13px sans-serif';ctx.textAlign='center';const ls=lines(label,26).slice(0,4);ls.forEach((t,i)=>ctx.fillText(t,x+w/2,y+h/2-(ls.length-1)*8+i*16));}" ++
  "function arrow(x1,y1,x2,y2,label,color){ctx.strokeStyle=color||'#93c5fd';ctx.fillStyle=color||'#93c5fd';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(x1,y1);ctx.lineTo(x2,y2);ctx.stroke();const a=Math.atan2(y2-y1,x2-x1);ctx.beginPath();ctx.moveTo(x2,y2);ctx.lineTo(x2-12*Math.cos(a-.45),y2-12*Math.sin(a-.45));ctx.lineTo(x2-12*Math.cos(a+.45),y2-12*Math.sin(a+.45));ctx.closePath();ctx.fill();ctx.font='11px sans-serif';ctx.textAlign='center';ctx.fillText(label,(x1+x2)/2,(y1+y2)/2-7);}" ++
  ...
  "function render(){ctx.fillStyle='#111';ctx.fillRect(0,0,cv.width,cv.height);title.textContent=model.label||model.task||model.id||'Current draft';summary.textContent='Persistent browser model: '+(model.actors||[]).length+' actors, '+(model.messages||[]).length+' visible messages, '+(model.properties||[]).length+' properties.';" ++
  "const actors=model.actors||[],msgs=(model.messages||[]).map(msgParts),props=model.properties||[],reducers=model.reducers||[];group(24,24,1052,82,'subject');rect(44,48,300,40,(model.label||model.task||model.id||'draft'),'#22c55e');rect(388,48,270,40,'observable messages only','#93c5fd');rect(704,48,330,40,'Lean artifact + CTL checks','#fbbf24');" ++
  "group(24,132,250,430,'actors');actors.forEach((a,i)=>rect(52,172+i*66,194,42,a,'#93c5fd'));" ++
  "group(310,132,450,430,'task / protocol message sequence');msgs.forEach((m,i)=>{const y=172+i*58;rect(342,y,170,38,m.src+' -> '+m.dst,'#fff');rect(548,y,176,38,m.proto,'#fbbf24');arrow(512,y+19,548,y+19,'fields','#777');if(i<msgs.length-1)arrow(636,y+42,636,y+58,'next','#555');});" ++
  "group(800,132,276,430,'properties and reducers');props.slice(0,5).forEach((p,i)=>rect(828,172+i*58,220,38,p,'#22c55e'));reducers.slice(0,3).forEach((r,i)=>rect(828,472+i*36,220,28,r,'#a78bfa'));" ++
  "actors.forEach((a,i)=>{const y=172+i*66+21;for(const [j,m] of msgs.entries()){const my=172+j*58+19;if(m.src===a)arrow(246,y,342,my,'send','#315f92');if(m.dst===a)arrow(342,my,246,y,'recv','#f59e0b');}});" ++
  "}" ++
  "function save(next){model=Object.assign({},empty,next);localStorage.setItem('leanfm.subject',JSON.stringify(model));render();}" ++
  ...
  "window.LeanFMUI={setSubject:save,renderText:textCard,renderLinks:linkCard,getSubject:()=>model};render();})();" ++
  "</script>"
