...
def liveSubjectGraph : String :=
  ...
  "function stepForce(){const by=new Map(nodes.map(n=>[n.id,n])),cx=cv.width/2;for(const n of nodes){if(n.pinned)continue;n.vx+=(cx-n.x)*0.0005;n.vy+=(170+n.rank*280-n.y)*0.0024;}for(const e of edges){const a=by.get(e[0]),b=by.get(e[1]);if(!a||!b)continue;const as=groupSize(a),bs=groupSize(b),dx=b.x-a.x,dy=b.y-a.y,d=Math.max(1,Math.sqrt(dx*dx+dy*dy));const targetY=(as.h+bs.h)/2+220,targetX=Math.max((as.w+bs.w)/2+190,340);const fy=(dy-targetY)*0.014,fx=(Math.abs(dx)-targetX)*0.0015*Math.sign(dx||1);if(!a.pinned){a.vy+=fy;a.vx+=fx;}if(!b.pinned){b.vy-=fy;b.vx-=fx;}if(b.y<a.y+targetY){const push=(a.y+targetY-b.y)*0.036;if(!a.pinned)a.vy-=push;if(!b.pinned)b.vy+=push;}}for(let i=0;i<nodes.length;i++)for(let j=i+1;j<nodes.length;j++){const a=nodes[i],b=nodes[j],as=groupSize(a),bs=groupSize(b);let dx=b.x-a.x,dy=b.y-a.y;if(Math.abs(dx)<1&&Math.abs(dy)<1){dx=7;dy=5;}const d2=Math.max(1,dx*dx+dy*dy),d=Math.sqrt(d2),near=Math.max((as.w+bs.w)/2+230,(as.h+bs.h)/2+210),rep=(near*near*0.32)/d2,rx=rep*dx/d,ry=rep*dy/d;if(!a.pinned){a.vx-=rx;a.vy-=ry;}if(!b.pinned){b.vx+=rx;b.vy+=ry;}}separateRects();for(const n of nodes){if(n.pinned){n.vx=0;n.vy=0;}else{n.vx=Math.max(-22,Math.min(22,n.vx*0.38));n.vy=Math.max(-22,Math.min(22,n.vy*0.38));n.x+=n.vx;n.y+=n.vy;}clamp(n);}separateRects();}" ++
  ...
  "function render(){if(!nodes.length)build();resizeCanvas();stepForce();ctx.fillStyle='#111';ctx.fillRect(0,0,cv.width,cv.height);title.textContent=model.label||model.task||model.id||'Current draft';summary.textContent='Persistent browser model: '+(model.actors||[]).length+' actors, '+(model.messages||[]).length+' visible messages, '+(model.properties||[]).length+' properties. Click composite states to open or close them.';hit=[];const by=new Map(nodes.map(n=>[n.id,n]));for(const e of edges){const a=by.get(e[0]),b=by.get(e[1]);if(a&&b)drawArrowLabel(a,b,e[2],e[0]==='exchange'?'#fbbf24':'#777');}for(const n of nodes)drawNode(n);requestAnimationFrame(render);}" ++
