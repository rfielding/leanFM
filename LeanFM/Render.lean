...
def aggregateGraphAnimation : String :=
  ...
  "function seed(id,i,total){const old=simById.get(id);if(old)return{x:old.x,y:old.y,vx:old.vx||0,vy:old.vy||0,pinned:!!old.pinned,pinX:old.pinX||old.x,pinY:old.pinY||old.y};const r=190+40*total,a=-Math.PI/2+i*Math.PI*2/Math.max(1,total);return{x:graphCanvas.width/2+Math.cos(a)*r,y:graphCanvas.height/2+Math.sin(a)*r,vx:0,vy:0,pinned:false};}" ++
