...
def liveSubjectGraph : String :=
  "<section><h2>Current Requirement</h2><div class=\"subjectPanel\"><canvas id=\"subjectGraph\" width=\"1180\" height=\"720\"></canvas><div><h3 id=\"subjectTitle\">No active draft</h3><p id=\"subjectSummary\">Ask the assistant to sketch a protocol or task.</p><div id=\"subjectArtifacts\" class=\"artifactPanel\"></div></div></div></section>" ++
  "<script>" ++
  "(()=>{const cv=document.getElementById('subjectGraph'),ctx=cv.getContext('2d'),title=document.getElementById('subjectTitle'),summary=document.getElementById('subjectSummary'),artifacts=document.getElementById('subjectArtifacts');" ++
  "const empty={id:'empty',task:'No active draft',actors:[],messages:[],properties:[],reducers:[]};let model=JSON.parse(localStorage.getItem('leanfm.subject')||'null')||empty;" ++
  ...
  "let openGroups=new Set(JSON.parse(localStorage.getItem('leanfm.subject.open')||'[\"exchange\"]'));let nodes=[],edges=[],hit=[],childById=new Map(),drag=null,started=false;" ++
  "function groupSize(n){if(n.id==='exchange'&&openGroups.has(n.id)){const c=Math.max(2,(model.messages||[]).length+2);return{w:520,h:Math.max(280,90+c*54)};}if(n.id==='annotations')return{w:290,h:260};return{w:230,h:82};}" ++
  ...
  "function build(){const msgs=(model.messages||[]).map(msgParts);const old=new Map(nodes.map(n=>[n.id,n]));function node(id,label,rank,color,x){const o=old.get(id);return{id,label,rank,color,x:o?o.x:x,y:o?o.y:110+rank*170,vx:o?o.vx:0,vy:o?o.vy:0,pinned:o?o.pinned:false};}nodes=[node('pre','preconditions',0,'#93c5fd',210),node('exchange','message exchange',1,'#fbbf24',520),node('terminal','terminal / cleanup',2,'#22c55e',520),node('annotations','properties / reducers',1,'#a78bfa',930)];edges=[['pre','exchange',msgs[0]?msgs[0].src+' -> '+msgs[0].dst+' '+msgs[0].proto:'entry'],['exchange','terminal','terminal cleanup / task completes'],['exchange','annotations','CTL over visible states']];}" ++
  "function clamp(n){const s=groupSize(n);n.x=Math.max(s.w/2+28,Math.min(cv.width-s.w/2-28,n.x));n.y=Math.max(s.h/2+28,Math.min(cv.height-s.h/2-28,n.y));}" ++
  "function stepForce(){const by=new Map(nodes.map(n=>[n.id,n]));for(const n of nodes){if(n.pinned)continue;n.vx+=(cv.width/2-n.x)*0.0008;n.vy+=(110+n.rank*190-n.y)*0.004;}for(const e of edges){const a=by.get(e[0]),b=by.get(e[1]);if(!a||!b)continue;const as=groupSize(a),bs=groupSize(b),dx=b.x-a.x,dy=b.y-a.y,d=Math.max(1,Math.sqrt(dx*dx+dy*dy));const target=Math.max(260,(as.h+bs.h)/2+95);const f=(d-target)*0.006,fx=f*dx/d,fy=f*dy/d;if(!a.pinned){a.vx+=fx;a.vy+=fy;}if(!b.pinned){b.vx-=fx;b.vy-=fy;}}for(let i=0;i<nodes.length;i++)for(let j=i+1;j<nodes.length;j++){const a=nodes[i],b=nodes[j],as=groupSize(a),bs=groupSize(b);let dx=b.x-a.x,dy=b.y-a.y;if(Math.abs(dx)<1&&Math.abs(dy)<1){dx=7;dy=5;}const ox=(as.w+bs.w)/2+56-Math.abs(dx),oy=(as.h+bs.h)/2+56-Math.abs(dy);if(ox>0&&oy>0){if(ox<oy){const m=ox/2+4,sgn=Math.sign(dx||1);if(!a.pinned)a.x-=m*sgn;if(!b.pinned)b.x+=m*sgn;}else{const m=oy/2+4,sgn=Math.sign(dy||1);if(!a.pinned)a.y-=m*sgn;if(!b.pinned)b.y+=m*sgn;}}}for(const n of nodes){if(n.pinned){n.vx=0;n.vy=0;}else{n.vx*=0.62;n.vy*=0.62;n.x+=n.vx;n.y+=n.vy;}clamp(n);}}" ++
  "function childState(parent,id,i,total){const key=parent.id+'|'+id;let c=childById.get(key);const s=groupSize(parent),tx=parent.x,ty=parent.y-s.h/2+76+i*54;if(!c){c={id,key,x:tx,y:ty,vx:0,vy:0};childById.set(key,c);}c.vx+=(tx-c.x)*0.03;c.vy+=(ty-c.y)*0.03;for(const other of childById.values()){if(other===c||!other.key.startsWith(parent.id+'|'))continue;let dx=c.x-other.x,dy=c.y-other.y;if(Math.abs(dx)<1&&Math.abs(dy)<1){dx=4;dy=3;}const d2=Math.max(1,dx*dx+dy*dy),d=Math.sqrt(d2),rep=9000/d2;c.vx+=rep*dx/d;c.vy+=rep*dy/d;}c.vx*=0.62;c.vy*=0.62;c.x+=c.vx;c.y+=c.vy;c.x=Math.max(parent.x-s.w/2+150,Math.min(parent.x+s.w/2-150,c.x));c.y=Math.max(parent.y-s.h/2+70,Math.min(parent.y+s.h/2-32,c.y));return c;}" ++
  "function drawNode(n){const s=groupSize(n),x=n.x-s.w/2,y=n.y-s.h/2;ctx.fillStyle=n.id==='exchange'&&openGroups.has(n.id)?'#111827':'#000';ctx.strokeStyle=n.color;ctx.lineWidth=2;ctx.fillRect(x,y,s.w,s.h);ctx.strokeRect(x,y,s.w,s.h);ctx.fillStyle='#fff';ctx.font='15px sans-serif';ctx.textAlign='center';ctx.fillText(n.label,n.x,y+26);if(n.id==='exchange'){ctx.font='12px sans-serif';ctx.fillText(openGroups.has(n.id)?'open: click to collapse':'closed: click to expand',n.x,y+47);}hit.push({id:n.id,x,y,w:s.w,h:s.h});if(n.id==='annotations'){const actors=model.actors||[],props=model.properties||[],reducers=model.reducers||[];rect(x+22,y+56,s.w-44,38,'actors: '+actors.join(', '),'#93c5fd');props.slice(0,2).forEach((p,i)=>rect(x+22,y+110+i*48,s.w-44,36,p,'#22c55e'));reducers.slice(0,1).forEach((r,i)=>rect(x+22,y+210+i*38,s.w-44,30,r,'#a78bfa'));}if(n.id==='exchange'&&openGroups.has(n.id)){const msgs=(model.messages||[]).map(msgParts);const states=['start'].concat(msgs.map(m=>'after '+m.proto)).concat(['terminal']);const kids=states.map((st,i)=>childState(n,st,i,states.length));kids.forEach((c,i)=>rect(c.x-130,c.y-20,260,40,states[i],i===0?'#93c5fd':i===states.length-1?'#22c55e':'#fbbf24'));msgs.forEach((m,i)=>{const a=kids[i],b=kids[i+1];arrow(a.x,a.y+22,b.x,b.y-22,'#fbbf24');labelBox(n.x+s.w/2-150,(a.y+b.y)/2,m.src+' -> '+m.dst+' '+m.proto,'#fbbf24',270);});}}" ++
  "function render(){ctx.fillStyle='#111';ctx.fillRect(0,0,cv.width,cv.height);title.textContent=model.label||model.task||model.id||'Current draft';summary.textContent='Persistent browser model: '+(model.actors||[]).length+' actors, '+(model.messages||[]).length+' visible messages, '+(model.properties||[]).length+' properties. Click composite states to open or close them.';if(!nodes.length)build();stepForce();hit=[];const by=new Map(nodes.map(n=>[n.id,n]));for(const e of edges){const a=by.get(e[0]),b=by.get(e[1]);if(a&&b)drawArrowLabel(a,b,e[2],e[0]==='exchange'?'#fbbf24':'#777');}for(const n of nodes)drawNode(n);requestAnimationFrame(render);}" ++
  ...
  "function point(ev){const r=cv.getBoundingClientRect();return{x:(ev.clientX-r.left)*cv.width/r.width,y:(ev.clientY-r.top)*cv.height/r.height};}function hitAt(p){return hit.find(h=>p.x>=h.x&&p.x<=h.x+h.w&&p.y>=h.y&&p.y<=h.y+h.h);}cv.addEventListener('pointerdown',ev=>{const p=point(ev),h=hitAt(p);if(!h)return;const n=nodes.find(x=>x.id===h.id);if(!n)return;drag={node:n,offX:p.x-n.x,offY:p.y-n.y,moved:false};n.pinned=true;cv.setPointerCapture(ev.pointerId);});cv.addEventListener('pointermove',ev=>{if(!drag)return;const p=point(ev);drag.moved=true;drag.node.x=p.x-drag.offX;drag.node.y=p.y-drag.offY;clamp(drag.node);});cv.addEventListener('pointerup',ev=>{if(drag){try{cv.releasePointerCapture(ev.pointerId);}catch(e){}if(!drag.moved&&drag.node.id==='exchange'){openGroups.has('exchange')?openGroups.delete('exchange'):openGroups.add('exchange');localStorage.setItem('leanfm.subject.open',JSON.stringify([...openGroups]));}drag=null;}});cv.addEventListener('dblclick',ev=>{const p=point(ev),h=hitAt(p);if(!h)return;const n=nodes.find(x=>x.id===h.id);if(n)n.pinned=false;});" ++
