...
def liveSubjectGraph : String :=
  ...
  "function groupChildren(n){const msgs=(model.messages||[]).map(msgParts),props=model.properties||[],reducers=model.reducers||[];if(n.id==='pre')return['entry request','auth/proof preconditions','queue/readiness'];if(n.id==='exchange')return['state 0: ready'].concat(msgs.map((m,i)=>'state '+(i+1)+': after '+m.proto)).concat(['terminal observation']);if(n.id==='terminal')return['postcondition','cleanup task','terminal observation'];if(n.id==='annotations')return['actors: '+(model.actors||[]).join(', ')].concat(props.slice(0,3)).concat(reducers.slice(0,2));return[];}" ++
